import type { ArtStyle } from '@/types/story';

export const artStyles: ArtStyle[] = [
  // ── Realistic: character clearly recognizable ──
  {
    id: 'watercolor',
    name: { en: 'Watercolor', de: 'Aquarell', fr: 'Aquarelle', it: 'Acquerello' },
    emoji: '🎨',
    image: '/images/water color style.jpg',
    description: {
      en: 'Soft watercolor painting with flowing colors',
      de: 'Sanfte Aquarellmalerei mit fliessenden Farben',
      fr: 'Peinture aquarelle douce aux couleurs fluides',
      it: 'Delicata pittura ad acquerello con colori fluidi',
    },
    prompt: 'Traditional watercolor painting, textured paper, delicate color washes (wet-on-wet technique), soft edges, visible artistic brushstrokes, transparent and flowing colors',
    category: 'realistic',
  },
  {
    id: 'realistic',
    name: { en: 'Realistic', de: 'Realistisch', fr: 'Réaliste', it: 'Realistico' },
    emoji: '📷',
    image: '/images/Real person.jpg',
    description: {
      en: 'Photorealistic illustration with natural lighting',
      de: 'Fotorealistische Illustration mit natürlichem Licht',
      fr: 'Illustration photoréaliste avec éclairage naturel',
      it: 'Illustrazione fotorealistica con luce naturale',
    },
    prompt: 'Photorealistic children\'s book illustration, natural lighting, cinematic composition, real-world textures, shallow depth of field, professional photography aesthetic',
    category: 'realistic',
  },
  {
    id: 'concept',
    name: { en: 'Concept Art', de: 'Konzeptkunst', fr: 'Art conceptuel', it: 'Arte concettuale' },
    emoji: '✨',
    image: '/images/concept art style.jpg',
    description: {
      en: 'Professional digital concept art',
      de: 'Professionelle digitale Konzeptkunst',
      fr: 'Art conceptuel numérique professionnel',
      it: 'Arte concettuale digitale professionale',
    },
    prompt: 'Highly detailed digital concept art, dramatic lighting, epic composition, smooth rendering, focus on mood and atmosphere, wide-angle lens, matte painting aesthetic',
    category: 'realistic',
  },
  {
    id: 'oil',
    name: { en: 'Oil Painting', de: 'Ölgemälde', fr: 'Peinture à l\'huile', it: 'Pittura a olio' },
    emoji: '🖼️',
    image: '/images/oil painting style.jpg',
    description: {
      en: 'Classic oil painting with rich textures',
      de: 'Klassisches Ölgemälde mit reichen Texturen',
      fr: 'Peinture à l\'huile classique aux textures riches',
      it: 'Pittura a olio classica con texture ricche',
    },
    prompt: 'Classic oil painting style, visible impasto brushstrokes, rich texture, heavy pigment, chiaroscuro lighting, canvas texture, museum quality art',
    category: 'realistic',
  },

  // ── Illustrated: stylized but recognizable characters ──
  {
    id: 'pixar',
    name: { en: 'Pixar 3D', de: 'Pixar 3D', fr: 'Pixar 3D', it: 'Pixar 3D' },
    emoji: '🎬',
    image: '/images/pixar art style 2.jpg',
    description: {
      en: 'Vibrant, warm Disney/Pixar style 3D animation',
      de: 'Lebendige, warme Disney/Pixar 3D-Animation',
      fr: 'Animation 3D Disney/Pixar vibrante et chaleureuse',
      it: 'Animazione 3D Disney/Pixar vivace e calorosa',
    },
    prompt: '3D render, Pixar cinematic style, natural eye proportion, subtle expression, highly detailed textures, octane render, volumetric lighting, vibrant colors, smooth shading, family-friendly',
    category: 'illustrated',
  },
  {
    id: 'cartoon',
    name: { en: 'Cartoon', de: 'Cartoon', fr: 'Dessin animé', it: 'Cartone animato' },
    emoji: '🎨',
    image: '/images/cartoon style.jpg',
    description: {
      en: 'Classic 2D cartoon style with bold colors',
      de: 'Klassischer 2D-Cartoon-Stil mit kräftigen Farben',
      fr: 'Style cartoon 2D classique aux couleurs vives',
      it: 'Classico stile cartone animato 2D con colori vivaci',
    },
    prompt: '2D cartoon style, bold black outlines, vibrant flat colors (minimal shading/cel-shading), smooth vector quality, classic Saturday morning animation aesthetic',
    category: 'illustrated',
  },
  {
    id: 'comic',
    name: { en: 'Comic Book', de: 'Comic', fr: 'Bande dessinée', it: 'Fumetto' },
    emoji: '💥',
    image: '/images/comic book style.jpg',
    description: {
      en: 'Comic book style with bold lines and halftone',
      de: 'Comic-Stil mit kräftigen Linien und Raster',
      fr: 'Style bande dessinée avec lignes épaisses et trames',
      it: 'Stile fumetto con linee marcate e retinatura',
    },
    prompt: 'A Franco-Belgian comic book illustration in the ligne claire style of Hergé and Peyo. Bold clean black outlines of uniform weight, flat solid colors, no halftone dots, no CMYK color separation, no crosshatching, no painterly shading. Bright friendly palette, dynamic composition. Not photorealistic.',
    category: 'illustrated',
  },
  {
    id: 'anime',
    name: { en: 'Anime', de: 'Anime', fr: 'Anime', it: 'Anime' },
    emoji: '⭐',
    image: '/images/anime style.jpg',
    description: {
      en: 'Japanese anime style with expressive features',
      de: 'Japanischer Anime-Stil mit ausdrucksstarken Features',
      fr: 'Style anime japonais aux traits expressifs',
      it: 'Stile anime giapponese con tratti espressivi',
    },
    prompt: 'Modern digital anime art, high detail, expressive large eyes, dynamic composition, detailed cel-shading, vibrant palette, style of Makoto Shinkai',
    category: 'illustrated',
  },
  {
    id: 'manga',
    name: { en: 'Manga', de: 'Manga', fr: 'Manga', it: 'Manga' },
    emoji: '📚',
    image: '/images/manga style.jpg',
    description: {
      en: 'Japanese manga style, ink linework with selective color accents on characters',
      de: 'Japanischer Manga-Stil, Tuschelinien mit gezielten Farbakzenten auf Figuren',
      fr: 'Style manga japonais, encrage avec accents colorés sélectifs sur les personnages',
      it: 'Stile manga giapponese, disegno a inchiostro con accenti di colore selettivi sui personaggi',
    },
    prompt: 'Traditional manga style, Japanese comic art, intricate detailed ink linework, atmospheric screentones, dramatic lighting and composition. Backgrounds and scenery rendered in monochrome ink + screentones. Character clothing, hair, and key story objects rendered in their natural specified colors (this is "color spread" manga — promotional cover style, not pure black-and-white interior panels).',
    category: 'illustrated',
  },

  // ── Creative: artistic/experimental, character likeness secondary ──
  {
    id: 'steampunk',
    name: { en: 'Steampunk', de: 'Steampunk', fr: 'Steampunk', it: 'Steampunk' },
    emoji: '⚙️',
    image: '/images/steampunk style.jpg',
    description: {
      en: 'Victorian-era inspired with gears and brass',
      de: 'Von der viktorianischen Ära inspiriert mit Zahnrädern und Messing',
      fr: "Inspiré de l'ère victorienne avec engrenages et laiton",
      it: "Ispirato all'epoca vittoriana con ingranaggi e ottone",
    },
    prompt: 'Steampunk graphic novel illustration in the style of Sean Murphy. Warm brown and muted amber color palette with restrained saturation — earthy tones only. Clean linework with even shading and smooth gradients; visible leather grain and metal patina where the scene contains machines or Victorian architecture. Mechanical elements (gears, cogs, pipes, clockwork) appear only on machines, vehicles, and architecture the scene describes — never on people (no gears or mechanisms on skin, hair, faces, or garments). Characters wear exactly their described clothing — the style changes rendering, not garments. Realistic human faces with natural proportions, detailed eyes, and subtle expressions.',
    category: 'creative',
  },
  {
    id: 'cyber',
    name: { en: 'Cyberpunk', de: 'Cyberpunk', fr: 'Cyberpunk', it: 'Cyberpunk' },
    emoji: '🌃',
    image: '/images/cyber punk style.jpg',
    description: {
      en: 'Dark futuristic cyberpunk aesthetic',
      de: 'Dunkle futuristische Cyberpunk-Ästhetik',
      fr: 'Esthétique cyberpunk futuriste sombre',
      it: 'Estetica cyberpunk futuristica e oscura',
    },
    prompt: 'Cyberpunk aesthetic, neon reflections, rainy streets, chrome, dense complexity, high contrast, dark atmosphere, volumetric fog, graphic novel illustration',
    category: 'creative',
  },
  {
    id: 'chibi',
    name: { en: 'Chibi', de: 'Chibi', fr: 'Chibi', it: 'Chibi' },
    emoji: '🌸',
    image: '/images/chibi style.jpg',
    description: {
      en: 'Cute chibi style with big heads and small bodies',
      de: 'Niedlicher Chibi-Stil mit grossen Köpfen und kleinen Körpern',
      fr: 'Style chibi mignon avec grandes têtes et petits corps',
      it: 'Adorabile stile chibi con teste grandi e corpi piccoli',
    },
    prompt: 'Chibi style, super deformed (SD) proportions, massive head, tiny body, kawaii aesthetic, adorable, smooth illustration, minimalist detail',
    category: 'creative',
  },
  {
    id: 'pixel',
    name: { en: 'Pixel Art', de: 'Pixelkunst', fr: 'Pixel art', it: 'Pixel Art' },
    emoji: '👾',
    image: '/images/pixel style.jpg',
    description: {
      en: 'Retro 16-bit pixel art style',
      de: 'Retro 16-Bit Pixelkunst-Stil',
      fr: 'Style pixel art rétro 16 bits',
      it: 'Stile pixel art retro a 16 bit',
    },
    prompt: '16-bit pixel art style, low resolution, limited color palette, detailed sprite work, retro video game aesthetic, style of Final Fantasy VI',
    category: 'creative',
  },
  {
    id: 'lowpoly',
    name: { en: 'Low Poly', de: 'Low Poly', fr: 'Low Poly', it: 'Low Poly' },
    emoji: '🔷',
    image: '/images/low poly 3-D style.jpg',
    description: {
      en: 'Minimalist geometric 3D style',
      de: 'Minimalistischer geometrischer 3D-Stil',
      fr: 'Style 3D géométrique minimaliste',
      it: 'Stile 3D geometrico minimalista',
    },
    prompt: 'Low poly 3D model, isometric perspective, minimalist geometric shapes, vibrant solid colors, clear edges, retro video game aesthetic, style of Monument Valley',
    category: 'creative',
  },
];
