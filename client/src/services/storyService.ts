...
export const storyService = {
  ...
  async testModels(storyId: string, pageNumber: number, models: string[], options?: {
    ...
    compositeStrategy?: 'stratified' | 'uniform';
  }): Promise<{
    ...
    const response = await api.post<{
      ...
      `/api/stories/${storyId}/test-models/${pageNumber}`,
      {
        ...
        ...(options?.compositeStrategy && { compositeStrategy: options.compositeStrategy }),
      }
    );
    return response;
  },
  ...
  async repairCharacters(
    ...
    options?: { useMagicApiRepair?: boolean; autoSelect?: boolean; grokRepairMode?: 'blended' | 'cutout' | 'fullScene'; whiteoutTarget?: 'face' | 'body'; maxCharRepairPages?: number; useGeminiRepair?: boolean }
  ): Promise<{
    results: Array<{
      character: string;
      pagesRepaired: Array<{
