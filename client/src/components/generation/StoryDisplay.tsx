  ...
  const renderCharRepairButton = (pageNumber: number, bboxDetection?: { figures?: Array<{ name?: string }> } | null, overrideKeyProp?: string) => {
    ...
    const detectAndOpen = async () => {
      ...
      try {
        ...
        if (data) {
          if (data.bboxDetection) {
            ...
            if (found.length > 0) {
              ...
              alert(language === 'de' ? 'Keine Figuren im Bild erkannt.' : language === 'fr' ? "Aucun personnage détecté dans cette image." : language === 'it' ? 'Nessun personaggio rilevato nell\'immagine.' : 'No characters detected in this image.');
            }
          } else {
            alert(language === 'de' ? 'Keine Figuren im Bild erkannt.' : language === 'fr' ? "Aucun personnage détecté dans cette image." : language === 'it' ? 'Nessun personaggio rilevato nell\'immagine.' : 'No characters detected in this image.');
          }
        ...
        alert(language === 'de' ? 'Erkennung fehlgeschlagen.' : language === 'fr' ? "La détection a échoué." : language === 'it' ? 'Rilevamento non riuscito.' : 'Detection failed.');
      } finally {
    ...
    return (
      // h-full: as a grid item next to taller (wrapped-label) siblings the
      // wrapper stretches, and without it the button stuck to 52px at the top
      // leaving a dead zone below — one of the "uneven buttons" reports.
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setCharRepairPopover(null)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-5 space-y-5">
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase tracking-wide block mb-2">
                    {language === 'de' ? 'Figur' : language === 'fr' ? "Personnage" : language === 'it' ? 'Personaggio' : 'Character'}
                  </label>
                  <select
                ...
                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase tracking-wide block mb-2">
                    {language === 'de' ? 'Bereich' : language === 'fr' ? "Zone" : language === 'it' ? 'Area' : 'Target'}
                  </label>
                  <div className="flex gap-2">
                    ...
                    >
                      {language === 'de' ? 'Gesicht' : language === 'fr' ? "Visage" : language === 'it' ? 'Viso' : 'Face'}
                    </button>
                    <button
                    ...
                    >
                      {language === 'de' ? 'Körper' : language === 'fr' ? "Corps" : language === 'it' ? 'Corpo' : 'Body'}
                    </button>
                  </div>
                </div>
                {developerMode && (
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500 mt-1">
                      Auto: face→blended, body→full-scene crosshatch. Blended blurs the region and regenerates; Cut-out extracts the figure, inpaints, composites back.
                    </p>
                  </div>
                )}
              </div>
              <div className="px-5 pb-5 flex gap-3">
                ...
                >
                  {language === 'de' ? 'Reparieren' : language === 'fr' ? "Réparer" : language === 'it' ? 'Ripara' : 'Repair'} ({CHARACTER_REPAIR_COST} {language === 'de' ? 'Credits' : language === 'fr' ? "crédits" : language === 'it' ? 'crediti' : 'credits'})
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="mt-2 bg-indigo-50 border border-indigo-200 rounded-lg p-3 space-y-2">
          <div className="space-y-1">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name={`iterateMode-${pageNum}`} checked={iterateMode === 'fresh'} onChange={() => setIterateMode('fresh')} className="text-indigo-500" />
              <span className="text-sm text-gray-700">{language === 'de' ? 'Neue Generierung' : 'Fresh generation'}</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name={`iterateMode-${pageNum}`} checked={iterateMode === 'reference'} onChange={() => setIterateMode('reference')} className="text-indigo-500" />
              <span className="text-sm text-gray-700">{language === 'de' ? 'Aktuelles Bild als Referenz' : 'Use original as reference'}</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name={`iterateMode-${pageNum}`} checked={iterateMode === 'blackout'} onChange={() => setIterateMode('blackout')} className="text-indigo-500" />
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={reviewPrompt} onChange={e => { setReviewPrompt(e.target.checked); if (!e.target.checked) { setPreviewPromptText(null); setPreviewPromptPage(null); } }} className="text-indigo-500 rounded" />
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 bg-indigo-500 text-white px-3 py-1.5 rounded-lg text-sm font-semibold ${(reviewPrompt && previewPromptPage === pageNum) || isPreviewingPrompt ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-600'}`}
            >
                                ...
                                <div className="mt-1 space-y-1">
                                  {Object.entries(char.generatedOutfits).map(([pageNum, outfit]) => (
                                    <div key={pageNum} className="bg-white p-1.5 rounded text-xs">
                                      <span className="font-medium text-indigo-500">Page {pageNum}:</span>
          ...
          <div className="mt-6 max-w-5xl mx-auto">
            ...
            {isGenerating && (
              <div className="mt-3 text-center text-xs text-indigo-500 font-medium flex items-center justify-center gap-1.5 py-2 bg-indigo-50 rounded-lg border border-indigo-200">
                ...
                {language === 'de' ? 'Qualitätsprüfung läuft — Bearbeitung nach Abschluss möglich' : language === 'fr' ? "Contrôle qualité en cours — modification possible une fois terminé" : language === 'it' ? 'Controllo qualità in corso — modifica possibile al termine' : 'Quality checks running — editing available after completion'}
              </div>
            )}
              ...
              <div className="mt-3">
                ...
                {_onRegenerateCover && (
                  <button
                    ...
                    title={language === 'de' ? 'Szenenbeschreibung bearbeiten und von Grund auf neu generieren' : language === 'fr' ? "Modifiez la description de la scène et régénérez depuis le début" : language === 'it' ? 'Modifica la descrizione della scena e rigenera da zero' : 'Edit the scene description and regenerate from scratch'}
                  >
      ...
      {hasImages && story && (
        <div className="space-y-8 mt-8">
          <h3 className="text-2xl font-bold text-gray-800 text-center mb-6">
            {title || (storyLang === 'de' ? 'Deine Geschichte' : storyLang === 'fr' ? 'Votre histoire' : storyLang === 'it' ? 'La tua storia' : 'Your Story')}
          </h3>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                >
                  ...
                  <span className="text-xs opacity-80">
                    ({COVER_REGENERATION_COST} {language === 'de' ? 'Credits' : language === 'fr' ? "crédits" : language === 'it' ? 'crediti' : 'credits'})
                  </span>
                </button>
              </div>
            </div>
