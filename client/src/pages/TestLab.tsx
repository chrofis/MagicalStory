...
function BeatsScenesView({ result }: { result: ExperimentResult }) {
  ...
  const beats = (result.finalBeats || []) as { pageNumber: number; planLine: string }[];
              ...
              {revised && before ? (
                <div className="mt-1 grid gap-2 md:grid-cols-2">
                  <div className="rounded border border-gray-200 bg-white p-2">
                    ...
                    <div><span className="text-gray-400">PLAN</span> {before.planLine}</div>
                  </div>
                  <div className="rounded border border-indigo-200 bg-white p-2">
                    ...
                    <div><span className="text-gray-400">PLAN</span> {b.planLine}</div>
                  </div>
                </div>
              ) : (
                <>
                  <div className="mt-0.5"><span className="text-gray-500">PLAN</span> {b.planLine}</div>
                </>
              )}
            </div>
          );
        ...
        <>
          ...
          {result.paintinSetup && (
            <div className="mt-3 rounded-lg border bg-gray-50 p-3 text-xs text-gray-700">
              <div className="font-semibold mb-1">What was sent to the model</div>
              <div>
                <b>Input 1 (edited):</b> title crop {result.paintinSetup.cropPx}px ={' '}
                {result.paintinSetup.cropPctOfCover} of the cover, rendered at {result.paintinSetup.renderedAt}
                {' '}· crop margin {result.paintinSetup.marginPct}
              </div>
              <div>
                <b>Input 2 (context only):</b> {result.paintinSetup.contextRef ? 'full cover, 1024px — palette + lighting reference, not edited' : 'none'}
                {' '}· refs sent: {result.paintinSetup.refsSent}
              </div>
              <div>
                <b>Colour:</b> {result.paintinSetup.recolor ? 'model may choose from the artwork palette' : 'locked to the deterministic pick'}
                {result.paintinSetup.deterministicColour && <> ({result.paintinSetup.deterministicColour})</>}
                {' '}· <b>mask dilate:</b> {result.paintinSetup.dilatePx}px — only these pixels come from the model
              </div>
              {result.promptUsed && (
