...
const pageTexts: Record<string, {
  ...
  en: {
    ...
    themesSubtitle: 'Our best themes for this gift',
  ...
  de: {
    ...
    themesSubtitle: 'Unsere besten Themen für dieses Geschenk',
...
export default function GiftPage() {
  ...
  const gift = useMemo(() => {
    return giftPages.find(g => g.id === giftSlug) || null;
  }, [giftSlug]);
  ...
  const intro = gift.intro[lang] || gift.intro.en;
  ...
  const themeCards = gift.recommendedThemes
    .map(rt => {
      ...
      if (!info) return null;
      return { ...rt, name: info.name[lang] || info.name.en, emoji: info.emoji };
    })
      ...
      <div className="bg-white border-b border-stone-100">
        <div className="max-w-4xl mx-auto px-4 pt-4 pb-0">
          <nav className="flex items-center gap-1.5 text-sm text-stone-500">
            <Link to="/geschenk" className="hover:text-indigo-600 transition-colors">{t.breadcrumbRoot}</Link>
