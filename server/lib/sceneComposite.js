...
async function generateSceneComposite(opts) {
  ...
  if (!cleanBackgroundPrompt) throw new Error('cleanBackgroundPrompt required');
  ...
  const bgBuf = Buffer.from(bg.imageData.replace(/^data:image\/\w+;base64,/, ''), 'base64');
  debug.cleanBackground = bg.imageData;
