...
function buildHairBlock(character) {
  ...
  return `\n${hairBits.join(' ')} Reproduce the hair EXACTLY in every cell — same length, same color, same shape, same parting. The back-view cell (cell 4) must show the same hair, turned toward the right shoulder to reveal one eye and cheek. Do NOT invent a different cut or a different face.\n`;
}
...
function buildHeadRowPrompt(character = null) {
  const hairBlock = buildHairBlock(character);
  return `Image 1 shows the four camera angles for a head-shot row — use it ONLY for facing direction (cell 1 front, cell 2 three-quarter, cell 3 profile, cell 4 back view, turned further than Image 1 shows so one eye and cheek are visible); ignore its face and features, and never draw arrows.
...
Output a 1×4 grid: ONE row, four cells side by side, thin black vertical dividers, pure white background. Each cell is a HEAD-AND-SHOULDERS close-up of the SAME PERSON — the head, neck, and the top of the shoulders wearing the costume; a little of the shoulders and collar showing is good, no bare skin below the neck. Never crop the top of the head. Cell 1 front, cell 2 three-quarter, cell 3 profile, cell 4 back view with the head turned toward the right shoulder — one eye and cheek visible. Photographic / lifelike; identity from Image 2; hair, skin tone, and costume consistent with Image 3. No text, numbers, labels, arrows, or symbols.`;
}
