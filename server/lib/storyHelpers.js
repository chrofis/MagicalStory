...
function buildTextZoneInstruction(textPosition, textZoneDescription, areaPct) {
  ...
  return `**COMPOSITION — CALM ZONE:** The ${corner} of the image (roughly ${areaPct}) is reserved for a white text overlay. Render the surface in that region as: ${surface} — gentle gradient, minimal texture, low contrast, saturated colour preferred (deep blue, dark green, rich brown, saturated cobblestone). The region carries scene material only — the same surface as its surroundings, painted continuously across. Characters whose feet naturally rest on this surface are drawn in full; the text overlay sits in front of them. When scene geometry occludes a character's lower body (table, counter, rail, water, wall, parapet, sofa back), that occluding object is in frame so the figure reads as standing behind it. If a layout reference image is attached, the slightly darker grey region marks the calm zone — paint that area as continuous scene material (sky, wall, water, foliage, ground).`;
}
