...
async function runUnifiedRepairPipeline(rawImages, context, options = {}) {
  ...
  const [evaluations, entityReport] = await Promise.all([
    images().evaluateImageBatch(evalInputs, { concurrency: evalConcurrency, qualityModelOverride, visualBible, clothingRequirements: storyData?.clothingRequirements || null, artStyle, ...evalStoryMeta }),
  ...
  if (baselineEvalInputs.length > 0) {
    const baselineEvals = await images().evaluateImageBatch(baselineEvalInputs, { concurrency: evalConcurrency, qualityModelOverride, visualBible, clothingRequirements: storyData?.clothingRequirements || null, artStyle, ...evalStoryMeta });
  ...
  for (let round = 1; round <= roundLimit; round++) {
        ...
        if (recolourResults.length > 0) {
          ...
          try {
            recolourEvals = await images().evaluateImageBatch(
              ...
              { concurrency: evalConcurrency, qualityModelOverride, visualBible, clothingRequirements: storyData?.clothingRequirements || null, artStyle, ...evalStoryMeta }
            );
          } catch (err) {
    ...
    if (roundSuccess.length > 0) {
      ...
      const [freshEntityResult, evalsResult] = await Promise.allSettled([
        ...
        images().evaluateImageBatch(roundEvalInputs, { concurrency: evalConcurrency, qualityModelOverride, visualBible, clothingRequirements: storyData?.clothingRequirements || null, artStyle, ...evalStoryMeta }),
      ]);
  ...
  try {
    ...
    if (rescueEntries.length > 0) {
      ...
      const rescueEvals = await images().evaluateImageBatch(buildEvalInputs(rescueEntries), { concurrency: evalConcurrency, qualityModelOverride, visualBible, clothingRequirements: storyData?.clothingRequirements || null, artStyle, ...evalStoryMeta });
      ...
      if (!blockedNoAnchor && MODEL_DEFAULTS.styleRepairProduction && (styleConsistency.outliers?.length || 0) > 0) {
        ...
        for (const target of plan.targets) {
          ...
          try {
            ...
            //
            // ================================================================
            // THE INHERITED SCORE IS DELIBERATE. DO NOT "FIX" IT.
            // ================================================================
            // A style-repair version carries prevBest's score on purpose. This
            // path is NOT decided by finalScore — it is decided by
            // repairPageStyle's OWN comparative eval, `compareStyleProximity`,
            // which puts the original and the repaint side by side and ships
            // only on `better === 'after'` with nothing else changed. That
            // verdict is `rep.passedGate`, stored on the version below as
            // styleRepair.passedGate / styleComparison.
            //
            // Why not score the repaint on its own bytes like inpaint does: a
            // style repaint changes the MEDIUM, not the content. The ordinary
            // per-image quality score cannot see the thing being repaired —
            // two images can score identically while only one is in the right
            // style — so an independent score adds a number that is blind to
            // the defect and then lets it veto the gate that isn't.
            //
            // This was changed to an independent evaluateImageBatch by
            // 0a3049bce (2026-09-05), on the reasoning that a version sharing
            // its predecessor's score loses selectBestVersion's 'earliest'
            // tie-break. The owner REVERTED that on 2026-09-06: the tie is not
            // the selector for this path. An audit that queries
            // imageVersions[].finalScore and reports "style repair never
            // improves the score / can never ship" has read the wrong column —
            // this has now been raised as a false positive more than once.
            // See docs/SETTLED.md and the header of server/lib/styleRepair.js.
            // ================================================================
