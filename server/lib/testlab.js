 ...
 */
function ideaPremiseSkeleton(sigs, presenceRatio = 0.6) {
  if (sigs.length < 2) return [];
  const need = Math.max(2, Math.ceil(presenceRatio * sigs.length));
  const vocab = [...new Set(sigs.flat())];
  const scored = vocab
...
function groupIdeasByPremise(ideas, opts = {}) {
  const { coverage = 0.6, presenceRatio = 0.6, minSkeleton = 4, minShare = 0.3 } = opts;
  ...
  while (pool.length > 1) {
    ...
    const skeleton = ideaPremiseSkeleton(sigs, presenceRatio);
    const sizes = sigs.map(s => s.length).sort((a, b) => a - b);
    const median = sizes[Math.floor(sizes.length / 2)] || 1;
    if (skeleton.length < minSkeleton || skeleton.length / median < minShare) break;
    ...
    groups.push({ members: covered, premiseWords: skeleton });
    pool = pool.filter(e => !covered.includes(e));
  }
...
async function runTrialIdeaVarietyStage(target, { params = {}, promptOverride = null }) {
  ...
  for (const arm of ['local', 'fantasy']) {
    ...
    const groups = groupIdeasByPremise(ideas, { coverage: overlap });
