 ...
 * examples.
 *
 * 2026-09-23: the arc, the plan and the prose no longer state sizes except
 ...
 * "2+ scenes" gate is why single-scene secondaries had no reference and inherited
 * the primary's face. So `characterMinAppearances` defaults to 1 and there is no
 * name/identity filter: any entry the story put in `secondaryCharacters` gets its
 * own face. Locations/artifacts/animals/vehicles keep the recurring-element gate
 * (`minAppearances`, default 2) — a one-page prop/place doesn't justify a
 * reference-gen call.
 *
...
function getElementsNeedingReferenceImages(visualBible, minAppearances = 2, characterMinAppearances = 1, artifactMinAppearances = 1) {
  ...
  const checkEntries = (entries, type, minPages = minAppearances) => {
    for (const entry of entries || []) {
  ...
  // Every secondary character gets its own reference even on a single page
  // (owner: "they should be in the visual bible even in a single scene").
