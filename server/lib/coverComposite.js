...
async function generateCoverViaComposite({
  ...
  coverHint,
  landmarkBuf,
  // sceneBackground = already-styled manga/watercolor empty scene of the
  // landmark (generated by coverIterate's packReferences-equivalent block).
  // When present, the SINGLE-PASS branch runs: composite figures directly
  // onto this backdrop and run ONE Grok edit (blend + ground + title). No
  // style transfer needed (backdrop is already styled), no landmark
  // protection needed (landmark is already correctly placed and won't be
  // touched by sharp.composite). When null, falls back to the legacy
  // two-pass flow: figures-on-white → Grok pose edit → cutout → composite
  // onto raw landmark photo → Grok style-transfer + landmark-protection
  // edit (which historically dropped the landmark every iterate round).
  ...
  // 4. Composite figures + prop. Base layer = sceneBackground (already-styled
  // manga/watercolor empty scene) when provided, else WHITE plate (legacy
  // two-pass behavior). Either way we composite figures + prop at the SAME
  // bottom-centered positions onto a 1024×1365 canvas.
  let baseLayer;
  if (sceneBackground) {
    ...
    log.info(`🎨 [COVER-COMPOSITE] ${label}: using styled empty scene as base (single-pass mode)`);
  } else {
    baseLayer = await sharp({ create: { width: W, height: H, channels: 3, background: { r: 255, g: 255, b: 255 } } }).jpeg({ quality: 92 }).toBuffer();
  }
  const whiteBg = baseLayer; // name kept for the existing composite-layers code below
  const margin = 24;
  ...
  if (coverKey === 'backCover') {
    ...
    if (dedication) {
      textLine = `\nDEDICATION TEXT: render this exact dedication in the lower third of the canvas: "${dedication}". Hand-painted ${artStyle} letterforms — NOT a system font, kept flat and graceful (this is a dedication, not a title — quieter, smaller, no 3D depth). Do NOT render the story title. This is the only text in the image.`;
    } else {
    ...
    textLine = `\nTITLE TEXT: render this exact title across the upper third of the canvas: "${title}". Hand-painted ${artStyle} letterforms — NOT a system font, not flat digital text. Looks brushed by an illustrator. Letters have depth, integrated with the scene above the figures. Title goes in the sky / upper background area, never on faces. This is the only text in the image.`;
  }

  // Branch the pass-1 prompt by mode. Single-pass: figures sit on the already-
  // styled empty scene; we ask Grok to refine pose, blend edges, repaint
...
After fixing poses: BLEND the figures into the scene. Soften visible cutout edges around each figure so they look painted in, not pasted. Match the lighting direction of the background. REPAINT the ground patch directly beneath each figure's feet (a small area, only beneath/around feet) so it reads as a natural continuation of the surface the landmark stands on — do NOT extend the ground OVER the buildings, the river, the foliage, or other background elements.${textLine}
...
If any figure still has arms at their sides, the task has failed. If the background contains a scene (forest, street, building, sky with clouds, etc.) instead of staying plain, the task has failed.`;
  ...
  // entirely. With sceneBackground, the styled landmark is already baked into
  // the input — pass 1 did the blend + ground + text refinement in one call.
  if (sceneBackground || !landmarkBuf) {
    return {
  ...
  // single-pass branch) so we reuse the same value here.
