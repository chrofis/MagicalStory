          ...
          jsonb_build_object(
            ...
            'clothingReviewReport', CASE
              WHEN jsonb_typeof(data::jsonb->'clothingReviewReport') = 'object'
                THEN (data::jsonb->'clothingReviewReport') - 'prompt' - 'rawResponse'
            ...
            'storyType', data::jsonb->'storyType',
            'storyTypeName', data::jsonb->'storyTypeName',
            'storyCategory', data::jsonb->'storyCategory',
            'storyTopic', data::jsonb->'storyTopic',
            'storyTheme', data::jsonb->'storyTheme',
            'storyDetails', data::jsonb->'storyDetails',
            'dedication', data::jsonb->'dedication',
            'mainCharacters', data::jsonb->'mainCharacters',
            'relationships', data::jsonb->'relationships',
            'relationshipTexts', data::jsonb->'relationshipTexts',
            'generatedPages', data::jsonb->'generatedPages',
            'totalPages', data::jsonb->'totalPages',
            'totalScenes', data::jsonb->'totalScenes',
  ...
  try {
    ...
    if (separateImages && separateImages.length > 0) {
      ...
      const mergeFields = (target, source) => {
        ...
        target.semanticResult = source.semanticResult || null;
        target.fixableIssues = source.fixableIssues || [];
        ...
        if (Array.isArray(source.compositeAttempts)) {
          target.compositeAttempts = source.compositeAttempts.map(a => ({
            ...
            propRef: typeof a?.propRef === 'string' && !a.propRef.startsWith('data:') ? a.propRef : null,
...
router.get('/:id/share-status', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      shareUrl: is_shared && share_token ? `${process.env.FRONTEND_URL || process.env.BASE_URL || 'https://www.magicalstory.ch'}/s/${share_token}` : null
    });
  } catch (err) {
