#!/usr/bin/env node
/**
 * Refine round 15 — Tellsplatte: revert to v13 boat (bow-to-stone) and edit
 *                   ONLY the stone — natural shape, top slightly tilted down.
 */
...
const EDITS = [
  {
    ...
    style: 'watercolor-v15-stone-natural-slight-tilt',
    editPrompt: 'Edit only the projecting STONE in this scene — leave the boat completely untouched (do not change its position, orientation, size, or shape), and leave the cliff, lake, rain, sky, and watercolor style exactly as they are. Make the stone look more NATURAL: a weather-worn boulder with irregular, organic contours, rough natural texture, no engineered straight cuts. The top of the stone is still a usable platform but slightly TILTED DOWNWARD — a gentle slope where the high edge is on the left (against the cliff) and the surface tilts gently down toward the water on the right. Subtle tilt, not steep. Do not modify the boat. Single edge-to-edge illustration, no borders, no text.'
  },
  {
    slug: 'hohle-gasse-kuessnacht',
    sourceFile: 'ai-13-watercolor-v11-rider-2x-same-spot.jpg',
    style: 'watercolor-v12-rider-2x-again',
    editPrompt: 'Edit this Hohle Gasse forest ravine scene with one adjustment to the horse and rider only — leave everything else (V-ravine, mossy banks, animal trail, foreground hiding bushes, leaning beech trees, dappled light, watercolor style, A4 portrait) exactly as it is. DOUBLE THE SIZE of the horse and rider AGAIN (roughly 2x current scale, so about 4x the original tiny version), keeping them at THE EXACT SAME LOCATION at the far end of the ravine where the trail bends out of sight in the upper-right. Same pose: horse facing forward riding down the path toward the viewer, chestnut coat, dark-cloaked rider on top. Just larger. Single edge-to-edge illustration, no borders, no text.'
  }
];
...
(async () => {
  ...
  for (const e of EDITS) {
    console.log(`\n=== EDIT v15: ${e.slug} (${e.style}) base=${e.sourceFile} ===`);
