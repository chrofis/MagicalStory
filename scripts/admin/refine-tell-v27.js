#!/usr/bin/env node
/**
 * Refine round 27 — Boat at sea with mountain background, A4 portrait,
 *                   based on Bauen panorama (landscape) of Lake Uri.
 *                   Convert pano → portrait + add medieval rowboat.
 */
...
const EDITS = [
  {
    ...
    refSourcePath: path.join(OUT_DIR, 'tellsplatte-boat-jump', 'refs', 'bauen-pano.jpg'),
    style: 'watercolor-v20-boat-on-lake-from-bauen-pano',
    prompt: 'Use the supplied Bauen panorama photo of Lake Uri (Vierwaldstättersee) as the scenery reference. Convert it from landscape panorama to an A4 PORTRAIT 3:4 vertical illustration in 19th-century romantic Swiss watercolor style. Keep the mountain skyline and the lake from the photo exactly as they are — same peak silhouettes, same shoreline. To convert from panorama to portrait: stack the scene vertically — upper half is the dramatic sky and the snow-capped alpine peaks rising above the lake, the middle band is the lake surface stretching to the cliffs, the lower half is the open lake water in the foreground with visible waves and ripples. In the FOREGROUND of the lake place a single MEDIEVAL WOODEN ROWBOAT (broadside view, full hull visible, period 14th-century Swiss design with wooden planks, rough oars, no people inside) sailing across the calm-to-choppy lake water. The boat is the only foreground subject, surrounded by lake. Soft warm afternoon light, brown ink outlines, atmospheric haze in the distance. Single edge-to-edge illustration, no borders, no text.'
  }
];
...
(async () => {
  ...
  for (const e of EDITS) {
    console.log(`\n=== EDIT v27: ${e.slug} (${e.style}) base=${path.basename(e.refSourcePath)} ===`);
    ...
    try {
      const result = await editWithGrok(e.prompt, [refDataUri], { aspectRatio: '3:4' });
      ...
      if (loc) {
        loc.ai_candidates = [...(loc.ai_candidates || []), {
          file: fname, style: e.style, prompt: e.prompt, basedOn: path.basename(e.refSourcePath),
