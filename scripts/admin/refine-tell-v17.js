#!/usr/bin/env node
/**
 * Refine round 17 — Tellsplatte: lower the flat shelf so it aligns with the
 *                   top of the boat's gunwale.
 */
...
const EDITS = [
  {
    ...
    sourceFile: 'ai-18-watercolor-v16-stone-jagged-with-small-flat-shelf.jpg',
    style: 'watercolor-v17-flat-shelf-aligned-with-boat-top',
    editPrompt: 'Edit only the height of the SMALL FLAT SHELF on top of the projecting stone — leave the boat completely untouched (do not change its position, orientation, size, or shape), and leave the rest of the stone, the cliff, the lake, the rain, the stormy sky and the watercolor style exactly as they are. Lower the small flat shelf on the stone so its height ALIGNS with the TOP EDGE (gunwale) of the boat — when you look across from boat to stone, the flat shelf and the boat\'s upper rim sit at roughly the SAME horizontal level. The surrounding jagged rock stays where it is; only the flat landing portion drops down to boat-gunwale height. The shelf remains slightly tilted toward the water on the right. Do not modify the boat. Single edge-to-edge illustration, no borders, no text.'
  }
];
...
(async () => {
  ...
  for (const e of EDITS) {
    console.log(`\n=== EDIT v17: ${e.slug} (${e.style}) base=${e.sourceFile} ===`);
