#!/usr/bin/env node
/**
 * Refine round 38 — Armbrust: take ONLY the user-supplied OIP photo and
 *                   convert it to watercolor style. Nothing else changes.
 */
...
const EDITS = [
  {
    ...
    sourceFile: path.join(OUT_DIR, 'story-asset-armbrust', 'refs', 'oip-user-string-reference.jpg'),
    style: 'watercolor-v9-from-oip-watercolorized',
    editPrompt: 'Take this Armbrust and redraw it in 19th-century romantic Swiss watercolor style with brown ink outlines on a plain off-white watercolor background. Keep the weapon exactly the same — same shape, same proportions, same single bowstring, same orientation. Only the rendering changes from photograph to watercolor illustration. Single edge-to-edge illustration on plain off-white background, no borders, no text.'
  }
];
...
(async () => {
  ...
  for (const e of EDITS) {
    console.log(`\n=== EDIT v38: ${e.slug} (${e.style}) base=${path.basename(e.sourceFile)} ===`);
    if (!fs.existsSync(e.sourceFile)) { console.log(`  SKIP: ref not found`); continue; }
    const sourceBuf = fs.readFileSync(e.sourceFile);
    ...
    try {
      ...
      if (loc) {
        loc.ai_candidates = [...(loc.ai_candidates || []), {
          file: fname, style: e.style, prompt: e.editPrompt, basedOn: path.basename(e.sourceFile),
