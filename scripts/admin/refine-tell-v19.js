#!/usr/bin/env node
/**
 * Refine round 19 — Tellsplatte: lower the stone closer to the water + make
 *                   the waves stronger.
 */
...
const EDITS = [
  {
    ...
    sourceFile: 'ai-20-watercolor-v18-stone-abrupt-end-boat-fully-visible.jpg',
    style: 'watercolor-v19-stone-lower-stronger-waves',
    editPrompt: 'Edit this Tellsplatte scene with two adjustments — leave the cliff, the boat (do not change its position, orientation, size, or shape), the rain, the stormy sky and the watercolor style exactly as they are. 1) Move the projecting STONE slightly LOWER, closer to the water surface — the top of the stone now sits less high above the waterline, so the rock looks like a low natural shelf rising only a small distance out of the lake rather than a tall boulder. Keep its abrupt right edge and slight tilt toward the water. 2) Make the LAKE WAVES STRONGER and more dramatic — bigger whitecaps, more visible chop, splash where waves hit the base of the stone and the side of the boat, foam crests across the lake surface, sense of a turbulent storm sea rather than ripples. Heavier visual energy in the water. Single edge-to-edge illustration, no borders, no text.'
  }
];
...
(async () => {
  ...
  for (const e of EDITS) {
    console.log(`\n=== EDIT v19: ${e.slug} (${e.style}) base=${e.sourceFile} ===`);
