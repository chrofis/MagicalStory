#!/usr/bin/env node
/**
 * Refine round 33 — Armbrust from scratch, no references. Simple, no bolt,
 *                   no pointed objects.
 */
...
const GEN = {
  ...
  style: 'watercolor-v4-fresh-no-refs-simple-no-bolt',
  prompt: 'A simple 19th-century romantic Swiss watercolor illustration of a single medieval Armbrust (crossbow), with brown ink outlines on a plain off-white watercolor background. Side view, full weapon visible end-to-end, oriented horizontally across the canvas. Plain wooden stock, a steel prod (the bow) at the front end, a taut string between the two prod tips, a simple wooden trigger at the back. NO BOLT in the groove, NO arrow, NO pointed objects anywhere on or around the weapon. The weapon is empty / unloaded. Plain wood and steel only — no engravings, no carvings, no ornamentation, no Swiss crosses. Single edge-to-edge illustration, no borders, no text.'
};
...
(async () => {
  ...
  console.log(`\n=== GEN v33: ${GEN.slug} (${GEN.style}) — text-to-image, no refs ===`);
  const sceneDir = path.join(OUT_DIR, GEN.slug);
  const existing = fs.readdirSync(sceneDir).filter(f => /^ai-\d+-/.test(f));
  const idx = String(existing.length + 1).padStart(2, '0');
  try {
    const result = await generateWithGrok(GEN.prompt, { aspectRatio: '3:4', resolution: '1k' });
    if (!result?.imageData) { console.log('  FAILED'); process.exit(1); }
    const b64 = result.imageData.replace(/^data:image\/\w+;base64,/, '');
    const buf = Buffer.from(b64, 'base64');
    const fname = `ai-${idx}-${GEN.style}.jpg`;
    fs.writeFileSync(path.join(sceneDir, fname), buf);
    console.log(`  ${idx}-${GEN.style}: ${(buf.length / 1024).toFixed(0)}KB`);
    const loc = manifest.locations.find(l => l.slug === GEN.slug);
    if (loc) {
      loc.ai_candidates = [...(loc.ai_candidates || []), {
        file: fname, style: GEN.style, prompt: GEN.prompt,
        generatedBy: 'grok-imagine', bytes: buf.length,
      }];
    }
  } catch (err) {
