#!/usr/bin/env node
/**
 * Refine round 11 — Apple shot: boy further back (back touching tree),
 *                   more spectators on the left, no people right of the tree.
 */
...
const EDITS = [
  {
    ...
    sourceFile: 'ai-11-composition-v8-boy-facing-front-empty.jpg',
    style: 'composition-v9-boy-against-trunk-more-crowd',
    editPrompt: 'Edit this medieval Altdorf apple-shot scene with these adjustments. 1) Push the BOY further BACK so his BACK is touching the lime tree TRUNK — he stands flat against the trunk in the right background, his back pressed to the bark, still facing the camera with the apple balanced on his head. He is small in the frame because of the depth. 2) Add MORE SPECTATORS to the LEFT side of the image — increase the size of the clustered crowd on the left, more peasants, soldiers, townsfolk packed together as a tense informal cluster, still occupying only the LEFT third-to-half of the canvas. 3) Keep the right side around the tree COMPLETELY CLEAR of people — absolutely no figures right of the tree, no figures behind the tree, only the tree, the boy against its trunk, and the empty cobblestones around them. 4) Keep the empty front foreground (no figure in the bottom centre), the medieval timber-frame houses, snow-capped Uri Alps, watercolor style, brown ink outlines, soft warm afternoon light, A4 portrait composition. Single edge-to-edge illustration, no borders, no text.'
  }
];
...
(async () => {
  ...
  for (const e of EDITS) {
    console.log(`\n=== EDIT v11: ${e.slug} (${e.style}) base=${e.sourceFile} ===`);
