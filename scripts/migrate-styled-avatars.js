#!/usr/bin/env node
/**
 * Migration: Convert styledAvatars objects to strings
 *
 * Some legacy data has styledAvatars stored as {imageData, clothing} objects
...
async function migrateStyledAvatars() {
  console.log(`\n🔄 Styled Avatars Migration ${DRY_RUN ? '(DRY RUN)' : '(APPLYING CHANGES)'}\n`);
  ...
  try {
    ...
    if (DRY_RUN && totalFixed > 0) {
      ...
      console.log(`\n✅ No issues found - all styledAvatars are already strings.`);
    }
