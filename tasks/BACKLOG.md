...
- [ ] Entity check still reports NO clothing findings in the production configuration (head grid primary + body grid second). Re-measured on all 12 grids of prod job_1787689073034_1v6ew0y1kae: 0 clothing findings, 11 identity findings, 0 failed evals. The `clothing_check` enumeration works only on the body-primary path; pointing it at the full-body cells did not help. Earlier "verified 2 TP / 0 FP" was measured body-primary and does not hold. → docs/decisions.md 2026-08-26 "CORRECTION: the clothing enumeration was verified in a configuration production does not use"
