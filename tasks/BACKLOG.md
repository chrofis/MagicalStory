...
- [ ] **210 Swiss landmark rows have no image at all (measured 2026-08-28, OPEN).** Distinct from the 8 above: nothing to download in the first place. They are excluded by `HAS_PHOTO_SQL` so they never reach a story, but they also never will without a Commons gap-fill pass. → `scripts/admin/fetch-landmark-photos-free.js`
...
- [ ] **The auto-index trigger runs paid, unbounded work on a user request (2026-08-29, OPEN — needs the owner).** When index+cache miss and `discoverOnMiss` is true, `landmarkPhotos.js:2196` fires `indexLandmarksForCity(..., { analyzePhotos: true, maxLandmarks: 30 })` fire-and-forget: the 15s timeout bounds only the awaited discovery, not the background run. `storyIdeas.js` passes `discoverOnMiss: true`, so this is user-reachable. Measured 2026-08-29: **146 of 2,264 Swiss towns** resolve to zero and would trigger it (all of them HAVE rows — filtered out by class or by a judged score under 40, so judging turns served towns into triggering ones); every foreign location triggers it always. Options: leave it, cap it, or turn it off and let those towns fall back to proximity. → `docs/landmark-database.md` §8
