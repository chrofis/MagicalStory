...
- [ ] **INVALIDATED 2026-09-11 — the "scene review has a NULL effect on render hazards" finding (Lab 1107-1134) rests on truncated reviews.** The Lab harness capped the scene reviewer at 16,000 output tokens (production: model max); 6 of the 8 redo arms hit exactly 16,000, two (1109, 1121) returned 0 chars and rewrote nothing, two (1122, 1124) were cut mid-output, and the harness stored `fromBeats` as the "reviewed" brief on every untouched page, so the raw→reviewed comparison compared raw with mostly-raw. The 27-of-64-edited / +0 vs +5 numbers are void; the review's effect on hazards is UNMEASURED, not null. Fixed (cap removed, truncation guard, hazard-count refusal, per-reviewer retention for a 5-reviewer bake-off via `sceneReviewModel=a,b,c` + `scene_hazard_count params.reviewer`) — see docs/decisions.md 2026-09-11 "Lab scene review was capped at 16k". Re-measure with the guard in place before any keep / drop / re-scope decision; a fair test judges the review on its OWN checks (cast/element counters raw vs reviewed) as well as hazards. → `server/lib/testlab.js` beats_scenes review call, `server/lib/sceneReviewGuard.js`, `prompts/scene-review.txt`
