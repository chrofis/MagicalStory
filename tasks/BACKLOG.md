...
- [ ] Subject pages (cast 0) hit two `length === 0 → use everyone` fallbacks: `entityConsistency.js:1587-1590` falls back to the whole story roster when a page's prose names nobody (one wasted detection call per subject page, and it asks the detector to find named children in a landscape), and `repairPipeline.js:283` (`wanted.size === 0`) picks every character as the style-reference pool. Both should be `length === 0 → skip`. → docs/decisions.md 2026-08-26 "Cross-story challenge memory + subject pages"
