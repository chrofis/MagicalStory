...
- [ ] **Landmark serving hands out a renovation photo: Stadtturm (Baden) primary photo is "Baden_Stadtturm_mit_Gerüst.jpg" (2026-09-06).** `landmark_index` id 8 (staging): `photo_url` is the Wikimedia scaffolding shot and its own `photo_description` says "undergoing renovation with protective scaffolding"; `photo_url_2` is a clean shot. Story `job_1788681313413_xqmtk2gcs` p13 + both covers reproduced the scaffold cages. Decision needed: demote/discard photos whose description states scaffolding/renovation/construction at judging time, or at serving time, or both — and re-check prod rows for the same words. → `server/lib/landmarkPhotos.js` serving ladder, `docs/landmark-database.md` judging model, `scripts/admin/*landmark*` judge
