...
- **A cast-0 page still gets no landmark PHOTO (2026-09-02, pre-existing, unchanged by the plate fix).** `refMode: 'off'` drops `landmarkPhotos`, so a landmark establishing shot with nobody in it reaches the model with the VB grid only — no curated photo. Before the cast-0 plate fix that page got literally nothing (the landmark rode the plate, and the plate was dropped), so this is strictly better, not a regression. Decide whether `'off'` should keep the landmark photo the same way it now keeps the plate — it is a place's identity, not a character's. Owner call. → `server/lib/clothingResolve.js` `applyReferenceMode`, docs/decisions.md 2026-09-02 "A cast-0 page gets no plate"
