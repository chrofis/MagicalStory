...
- [ ] **`Setting/location:` line makes the commission name the reader's home city as the binding world (found 2026-08-31, OPEN — owner decision).** `buildStoryContextFields` injects IP-geo `userLocation` inside the commission block under "What this names is binding: … the world it happens in", so a premise that names another world (e.g. a sea voyage abroad) gets relocated home — 2/2 pirate validation runs, all 3 judges flagged it; the 2026-08-31 landmark-precedence prompt fix cannot beat it. Fix candidate: relabel as reader's home for landmark use only, not the story's setting. → `server/lib/promptBuilders.js:4059`, `:4368`; docs/decisions.md 2026-08-31 "Arc-machine refinement 4"
