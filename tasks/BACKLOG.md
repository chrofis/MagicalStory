...
- [ ] **The trial (/try) has no season at all (2026-09-09).** The full wizard has a Jahreszeit selector whose value now reaches both idea cards; `prompts/trial-idea.txt` has no season placeholder and `server/routes/trial.js` never builds one, so a trial story's premise is season-blind and the illustrations take whatever `scene-expansion` defaults to. Not a regression — it was never built. Whether /try should infer the season from the current date (it is a zero-input funnel) or stay season-free is the owner's call. → `prompts/trial-idea.txt`, `server/routes/trial.js` idea generation, `prompts/scene-expansion.txt` `{SEASON}`
