...
- **`kill_workers()` can fire mid-inference — TOCTOU in `_maybe_reap_workers` (found by review 2026-09-05, PRE-EXISTING since `8d1f1c572`).** The busy check reads `_inflight_requests` under `_request_lock`, releases it, then `kill_workers()` acts on that stale answer. A request landing in the gap increments in-flight, calls `ensure_worker()` (worker still alive), issues its HTTP call — and is then terminated underneath, surfacing as a 502 from the worker call. Not introduced by the cache-drop work, but every cache drop rides on `kill_workers`, so it inherits the exposure. Fix is to hold the lock across the decision and the kill, or to mark workers "condemned" and let the last in-flight request release them. → `photo_analyzer.py` `_maybe_reap_workers` / `kill_workers`
- **`/api/health/memory` `python` field reports only the analyzer's ROUTER process (2026-09-04).** Workers hold the real memory; measured mid-run the router showed 89 MB while two workers held 1.43 GB and 985 MB, and the container peaked at 9.85 GB anon. The `cgroup` block added the same day is truthful, but the `python` field beside it still understates by >1 GB during work. → `server/routes/health.js`
- **Ten files still inline `process.env.PHOTO_ANALYZER_URL` instead of `photoAnalyzerClient.photoAnalyzerUrl()` (2026-09-04).** All honour the env var so the analyzer split works regardless; it is the duplication that `photoAnalyzerClient.js` was extracted to end. → `server/lib/imageCompositing.js`, `imageInpainting.js`, `referenceSheets.js`, `rembg.js`, `sceneComposite.js`, `entityConsistency.js`, `faceIdentity.js`, `analyzerClient.js`, `server/routes/avatars.js`, `server/routes/health.js`
