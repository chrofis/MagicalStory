...
- [ ] Entity check reports no clothing findings even when a named garment part is missing — measured on prod job_1787689073034_1v6ew0y1kae: Julian's dungaree bib+straps absent on p12 and all three covers, 6 findings returned, all hair/face, zero clothing. Not a perception limit (asked per-cell and per-part directly, Gemini answers all 5 cells correctly) and not plumbing (the contract text reaches the prompt verbatim). Three prompt edits — a part-checklist rule, a "spec outranks majority" clause, and a per-part step in the task list — each measured zero change and were NOT shipped. Next candidate: a dedicated narrow clothing pass over the body grid, the way the head grid was split out for identity. → docs/decisions.md 2026-08-26 "Inpaint may not be asked to change clothing, hair or face form"
