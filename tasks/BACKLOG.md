...
- [ ] Dragon-scale cell still reads leaf/petal after the kind sentence (Lab exps 1169/1170 — dish gone, gate NO twice). A lone reptile scale is ambiguous without a body; options to measure in the Lab: the bible's `type` naming the creature ("dragon scale" as type), a size/context clause in the description, or rendering the gate's NO reason back into the one re-render prompt (fed-back retry). → server/lib/referenceSheets.js `elementKindSentence`, docs/decisions.md 2026-09-11 element-cell entry
...
- [ ] **Dragon run 4 (job_1789147573901_m3uam0nxi, staging 2026-09-11) — 22 findings, all pages viewed. The theme: the pipeline scores its own defects and ships them anyway.** Gate class: a page at finalScore 0 (p6) and one at 5 (p9) are the ACTIVE versions with no warning anywhere; `unrepairedCritical` is populated on p13/p17 and those pages still report 85; the front cover shipped at 13 and its repair deleted a cast member; p5 (Julian missing + a headless disembodied dragon wing) scored semantic 100 with zero findings. Prop class: the bicycle lamp renders as a CCTV camera in 5/5 appearances (pistol grip on p16), the dragon scale renders as a LIVE DRAGON on p13 and p17, the dragon has no stable size. Print class: white paper borders on p1/p3/p6/p9/p11 against an explicit bleed rule. Cast: the whole cast renders as 2-year-olds while the text is 1st-grade with a declared-5 protagonist — the evaluator wrote this finding on four pages and nothing acted on it. Text: p10 (126 words) invents a three-way group split that is never paid off, the title promises a flightless dragon the story never delivers. NOTHING written to tasks/bugs.json — owner triages which of T1/T2/T4/T9 become push-blocking bugs; T2 and T10 are prompt-vs-code decisions needing sign-off first. → tasks/dragon-run4-findings-2026-09-11.md
