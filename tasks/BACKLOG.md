...
- [ ] **Body character repair on busy multi-figure pages: Grok redraws the WHOLE scene in every mode, blend gate (correctly) rejects all draws — repair unusable there (2026-09-01, OPEN).** job_1788215224103_avu132n7je p16 (6 figures): 12/12 draws rejected across fullScene (60-71 grey bg residual) AND cutout (36 residual, dx9/dy66/scale1.06 drift), before and after the template fix e7447ec81 — so this is model behaviour, not prompt selection. Candidate directions (owner decision): auto-fallback to face-only blended repair when the entity issue is facial (p16's issue is "appears older"); try useGeminiRepair for the body mode; a fallback ladder across modes. Do NOT loosen the gate — the drafts genuinely repaint the scene. → docs/decisions.md 2026-09-01 "FullScene crosshatch repair uses the scene template", scratchpad piraterun2/g7-editrepair/
