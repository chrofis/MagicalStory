...
- [x] Entity check now enumerates clothing per cell (`clothing_check`) before judging — fixed 2026-08-26, verified 9 grids: 2 true positives, 0 false negatives, 0 false positives. Open remainder: on the p8-p14 grid it reports a missing sneaker but still misses the same character's absent dungaree bib on p12, so one part-level miss survives. → docs/decisions.md 2026-08-26 "The entity grid enumerates clothing per cell before it judges"
