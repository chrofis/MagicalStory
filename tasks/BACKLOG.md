...
- [ ] **MEASURED 2026-09-11 — scene reviewer bake-off, 5 models on the fixed harness (Lab 1157-1188, $3.56): deepseek-v4-pro (production) justified, nobody worth switching to; owner decision open.** Supersedes the INVALIDATED 2026-09-11 "NULL effect" finding (Lab 1107-1134, truncated reviews). On identical 8-page AD briefs (dragon 1157, Bimo 1179) all five reviewers pass the truncation guard, clear every code-detected cast/interaction finding (7/7), leave every `vb_element_overflow` (5/5), and preserve `looksAt` on 100% of rewritten pages; the hazard judge moves −3 ± 3 over 12-13 rewritten pages for EVERY reviewer, and identical untouched text moves up to 3 — the judge cannot rank them. gemini re-introduces 3 `interaction_actor_unknown`; qwen takes 11-18 min; sol is 2-3.5x the cost under a 16,384 cap. Open for the owner: (a) keep deepseek (recommended, no change made); (b) all five reviewers give a SECONDARY its own `characters[]` row on counterpart pages (spec 8bc says `watching` interaction, no row) — harmless duplicate or downstream breakage?; (c) no reviewer ever trims Visual Bible elements to the grid. → docs/decisions.md 2026-09-11 "Scene REVIEWER bake-off, 5 models on the fixed harness"
