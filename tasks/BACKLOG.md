...
- [ ] **No render check compares detected figure count to declared cast size (2026-09-09).** Front cover of `job_1788903616404_iqvhj4l8m` showed FIVE children for a four-boy cast and scored 100/100. The evaluator is passed NO cast roster — only `ORIGINAL_PROMPT` text (`server/lib/evalPipeline.js:675`) — so it free-matched the invented fifth child to the name "Nia" it read in the prose, confidence 0.9, verdict PASS. The rubric penalises only MISSING (`D-04`) and DUPLICATE (`D-03`/`D-03b`) characters, and `N-09 "Crowd extras"` is an explicit NEVER-DEDUCT rule. Pairs with the `extra_character` orphan-type item above (2026-09-06): giving that type a prompt definition is necessary but NOT sufficient — without a roster the judge has nothing to count against. The name-leak that produced the figure is fixed (`53f7a9b62`), the detection gap is not. → `server/lib/evalPipeline.js:675` + `:854` `evaluateImageQuality`, `prompts/image-evaluation.txt:54` (N-09) / `:114` (D-04) / `:223` (matches spec), `server/lib/evalBuckets.js:137`
- [ ] **Repair dead band: a page with three MAJOR findings and no CRITICAL is structurally invisible to repair (2026-09-09).** `job_1788903616404_iqvhj4l8m` p3 shipped at finalScore 55 with zero repair attempts and three MAJORs (three-stage `clothing` jeans-not-joggers, three-stage `action_interaction` bread roll held at the waist instead of pocketed, semantic `missing_element` Levin's bicycle absent). `findBadPages` (`server/lib/repairLogic.js:30-70`) admits a page only on `score < 50` OR `issueCount >= 5` OR a CRITICAL; 55/2/none matched nothing. `enableFullRepair` was true and does reach the loop — it buys PASSES, not a lower bar. NOTE this is a different class from the closed 2026-09-04 entity ruling above (owner: MAJOR *entity* findings stay unrepaired by design); these are compliance/semantic findings, and the question of whether that class deserves admission is untouched. The 50 floor was itself lowered from 60 on 2026-08-09 because 50-59 repairs came back worse, so raising it is a reversal needing evidence. Owner's call. → `server/lib/repairLogic.js:30-70` + `:100`, `server/config/models.js:981-988` thresholds
- [ ] **The writer invents a parent with a fabricated face and no owner consent (2026-09-09).** `job_1788903616404_iqvhj4l8m` created secondary character `CHR001` "Mama", age 32, blonde wavy shoulder-length hair, green eyes, cornflower-blue linen dress — appearing on pages 2 and 18 and rendered full-figure embracing the child on p18. She is not one of the four uploaded characters; her entire appearance is invented. Owner raised it unprompted ("why is there a mother in it"). A real family's book showing an invented mother is a different failure class from an invented shopkeeper. Options (owner's call): forbid inventing a parent/guardian when the commissioned cast contains children, require such a role be filled from the uploaded cast or omitted, or keep it but never render the face. → `prompts/story-bible-from-beats.txt` secondaryCharacters spec, `server/lib/inventedAgeBand.js`, arc/beats invented-figure rules
- [ ] **The AD may plan a face-filling close-up for a page whose text carries the whole cast (2026-09-09).** `job_1788903616404_iqvhj4l8m` p7: the page text has four boys plus Nolo, polite asking, a refusal and a shouted outburst; the plan line chose `close-up — Max's face, chin lifted, mouth open mid-shout` and the render is a single screaming toddler filling the frame. It scored 100/100 because it matched its brief exactly — the defect is upstream of every evaluator. Owner reaction: "what is happening on page 3 / looks horrible" was p3, but p7 is the same class. Question is whether a close-up is admissible on a page whose text introduces a new character or resolves a group exchange. → `prompts/story-beats.txt` plan-line shot choice, `prompts/scene-expansion.txt` shot spec, `prompts/plan-check.txt`
- [ ] **`tests/manual/test-cover-prompt-builder.js` is stale — it asserts on prompt templates that no longer exist (2026-09-09).** `PROMPT_TEMPLATES.initialPageNoDedication` is `undefined` and the script throws before finishing; `prompts/` now contains only `cover-composition.txt` for covers — `front-cover.txt`, `back-cover.txt` and `initial-page-*.txt` are gone. Verified failing independently of the 2026-09-09 cover-name work (which touched no `prompts/*.txt`). CLAUDE.md's "Prompt Templates" section still lists those four deleted filenames and needs the same correction. → `tests/manual/test-cover-prompt-builder.js:143-166`, `server/services/prompts.js` template map, `CLAUDE.md` Prompt Templates list
