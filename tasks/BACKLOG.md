...
- [ ] **The scene review has a measured NULL effect on render hazards (2026-09-11, Lab 1107-1134).** Across 8 AD arms × 8 pages the DeepSeek scene review edited 27 of 64 briefs; on those 27 pages the hazard count (gpt-5.6-sol judge) moved by exactly 0, while the 37 UNTOUCHED pages moved by +5 — i.e. the whole raw→reviewed difference in the redo table is judge noise, not review effect. Its edits are light (a rewritten page grew 3101→3127 chars; opening text identical), and production ships the reviewed brief (`beatsPipeline.js:1552/1656/1795`). Cost ≈ $0.08 and ≈ 2 min per 8 pages, on the critical path (35-45% of the AD stage wall-clock). What it checks is mostly structural (cast lists, element budget, gaze budget, actor attribution) and no downstream measurement of those exists — so "null on hazards" ≠ "worthless", but its value is currently unmeasured. Owner decides keep / drop / re-scope; a fair test would judge the review on its OWN checks (e.g. cast/element counters on raw vs reviewed) rather than on hazards. → `server/lib/beatsPipeline.js` scene review call, `prompts/scene-review.txt`, docs/decisions.md 2026-09-10 redo entry
