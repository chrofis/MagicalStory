...
- **Trial dev panel preservation**: `clearStyledAvatarGenerationLog()` is called from TWO places in `server.js` (job-prep at ~2716 AND `_processStoryJobImpl` at ~7117). BOTH must skip the clear when `inputData.trialMode === true` — otherwise the second clear wipes the prepare-title styled-avatar log entries before the result captures them and the `StoryDisplay` "Stilisierte Avatare" dev panel renders empty.
- **Trial character physical traits reach Claude**: `buildTrialStoryPrompt` (`server/lib/storyHelpers.js`) now passes `character.physical` (`hairColor / eyeColor / skinTone / detailedHairAnalysis` — populated by `trial.js:737-745` after Gemini analyzes the uploaded photo) into the CHARACTERS section so scene prose can weave in real visual descriptors instead of bare action lines. Don't drop these when refactoring the prompt builder.
- **Invented secondaries get per-page descriptors**: `buildImagePrompt` injects a `**SECONDARY CHARACTERS IN THIS SCENE:**` block built from `visualBible.secondaryCharacters` filtered by `.pages[]`, even when `skipVisualBible: true` (the default for Grok). Without this, characters Claude invents (Mia/Noah/shopkeepers — no uploaded photo, no VB grid presence) reach Grok as bare names and get reinvented every page. Filter by `.pages[]` is load-bearing — including the full cast on every page blows Grok's 7500-char effective limit.
- **Landmark photo variants reach Claude**: `storyIdeas.js` maps `photo_url_N + photo_description_N` from `landmark_index` into `photoVariants: [{n, description}]` on each landmark. The trial landmarks instruction (`storyHelpers.js → buildTrialStoryPrompt`) emits a `PHOTO ANGLES` block per landmark when ≥2 variants exist, plus an explicit `[LOC###.N]` syntax hint. Holzbrücke (Baden) has variants 4-5 as interior shots — Claude picks them for "inside the bridge" scenes when prompted. Do NOT add specific landmark names as examples in the prompt (e.g. don't write `e.g. "Holzbrücke Baden"` as the `landmarkQuery` example) — Claude pattern-matches the example and picks it deterministically on every story in that city.
...
- **Avatar eval inputs normalized via `bytesFromAnyImage`**: `evaluateAvatarFaceMatch` (`server/routes/avatars.js`) now decodes both `originalPhoto` and `generatedAvatar` through `r2.bytesFromAnyImage()` before sending to Gemini. The old string `.replace(/^data:image\/\w+;base64,/, '')` path was a silent no-op for HTTPS R2 URLs (common post-R2 migration), causing the eval to fail in production and return null without logging. Don't revert.
