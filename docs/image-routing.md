...
|---|---|---|---|---|---|
| **Page (scene) image** | **Production: Grok `grok-imagine` ($0.02). Staging: Grok `grok-imagine-2` ($0.04).** Declared as `pageRenderModel` in `server/config/runtime.js`; read as `MODEL_DEFAULTS.pageRenderImage` / `simplePageImage` / `complexPageImage`. **Page redos and repair regenerations read the SAME key**, so a page cannot change tier mid-repair. | Always default | ✅ `grok-imagine` in production: $0.02, half of Gemini, "good enough" storybook. 🟡 `grok-imagine-2` on staging (2026-08-30) — **NO scored evidence yet**: Lab experiments 959/963/965 all ran `autoEval:false`, so their score arrays are empty and the basis is an eyeballed sample. Staging-only for exactly that reason; a scored A/B is what promotes it to production. | `image` (model override) | image_model_tests, decisions.md 2026-08-30 |
