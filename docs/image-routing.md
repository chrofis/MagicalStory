...
| **Avatar Round 2** (art/style transfer) | **Gemini** `gemini-2.5-flash-image` (pro→flash 2026-07-21, `AVATAR_STYLE_MODEL` reverts) | Always (shipped 2026-07-19) | ✅ real Pixar 3D; Grok barely stylises. `avatarStyleTransferBackend='gemini'`. **Failure chain (2026-07-31):** per-attempt throws (e.g. IMAGE_OTHER on adult-face sheets) are consumed retries → after 3 failed attempts ONE alternate-backend (Grok) retry → else ship the realistic Round-1 sheet unstyled → final `ensureStyledAvatarCoverage` backstop seeds a raw reference + loud warnings. A character can no longer end a story with zero avatars (decisions.md "Styled-avatar MUST guarantee"). Grok stays the FALLBACK, not the default — the all-5 A/B verdict stands. | `avatar_style` | image_model_tests 2026-07-19 |
...
| **Title INTO the artwork** (painted lettering, not a flat layer) | **glyph-conditioned paint-in**: composeCover renders real-font glyphs → diff-mask → crop → **Qwen** `qwen-image-edit@2511` styles the letters → mask-gated paste-back → **OCR exact-match gate** | Never let a model SPELL the title | 🟡 built 2026-08-05, NOT yet run. **❌ do not route titles to free generation** — 2026 SOTA is ~90-95% on short copy (Ideogram 4 best, GPT Image 2, Qwen) and worst on ä/ö/ü/é → ~1 in 10 DE/FR covers garbled, and a VLM judge misses single-letter errors | `cover_title_paintin` | `docs/cover-text-rendering-research.md` |
...
| **Style-repair** (repaint a style-outlier page OR cover toward the dominant cluster) | `repairPageStyle` (styleRepair.js) → `editImageWithPrompt` → default **Gemini** `gemini-2.5-flash-image` (`STYLE_REPAIR_MODEL=grok` → `grok-imagine`) | A page or cover flips art style (a `checkStoryStyleConsistency` outlier — the audit now includes all three covers at pages −1/−2/−3) | 🟡 **WIRED INTO PROD 2026-07-31** (owner directive; flag `styleRepairProduction`, env `STYLE_REPAIR_PRODUCTION`, default true): Step-5 audit → `planStyleRepair` → one repaint per outlier → `checkStyleMatch` gate → stored as new version + re-picked. Gemini-vs-Grok A/B still open in the Test Lab | `style_repair` | decisions.md Pt 10 + 2026-07-31 production wiring |
