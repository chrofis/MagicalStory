...
| **Garment hue normalize** (fix cloth colour drift) | `normalizeGarmentHue` (garmentHueNormalize.js) — **no model**, masked LAB hue-rotation toward the character's AVATAR garment colour | Runs PRE-EVAL (Phase 5b-hue) on every figure; corrects only outliers | ✅ lighting-preserving (rotate a*/b* about origin → L*+chroma exact); illumination-discounted so day/night survives; defers garment-TYPE errors to redraw. Flag `garmentHueNormalize` | `garment_hue` | decisions.md 2026-07-30 |
