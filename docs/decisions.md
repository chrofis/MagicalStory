...
## 2026-08-15 — SAM is told WHICH figure in the box is the target (occlusion)

**Context:** On `job_1786780194082_s980g4s9a` page −2 the garment recolour repainted **Emma's
correct red top olive and her correct orange shorts grey** (20,972 px + 12,510 px), while
Daniel — the character the fix was filed against — was already dressed correctly and was never
touched.

Daniel crouches directly behind Emma, so the person box drawn for him **encloses hers** (98.8%
containment). His entity cutout came back as **his head on her body**, the judge truthfully
reported "top is red, should be yellow" about those pixels, the recolour's DINO call found her
red shirt (the only top in the crop), and the colour gate confirmed the pixels really were red
before shifting them. Every stage was correct about a wrong input.

**The measured root cause is the opposite of the obvious one.** Replayed locally against
MobileSAM (all figures, $0):

| step | result |
|---|---|
| Emma's own box-only mask | 94,121 px — **and it covers 97% of DANIEL'S head zone** |
| existing carve-out (subtract her mask from his) | deletes **his own head** → the head-shaped void in the shipped cutout |
| erase her pixels to white, then SAM his box | SAM segments the white hole; **his face is gone** (10,871 px of fringe) |
| blur-fill instead of white | same failure, 5,565 px |
| geometry-only anchor points (no faces) | positive lands on background → shirt only, face lost |
| **box + POSITIVE on own face + NEGATIVE on each other face** | Emma's leak into his head zone **97% → 5.6%** |
| **+ subtract front figure's dilated mask + drop speckles** | **Daniel 25,524 px** — head, beard, yellow collar, chest, sandal. No Emma, no red. |

It was never "SAM grabbed the neighbour" — it was **Emma's mask containing Daniel's head**, which
is why every subtract-afterwards and erase-beforehand variant destroyed his face.

**Decision:** Box→mask runs through one shared masker, `_maskBoxesFrontFirst`, used by BOTH
detection paths. For each box it sends MobileSAM a positive point on that figure's own face and a
negative point on every other face falling inside the same box (`points` + `point_labels`, already
supported by `/figure-mask`). Boxes resolve smallest-first, and any already-resolved figure whose
box is ≥80% contained and strictly smaller (<90% area) is then subtracted (dilated 3 px, the two
masks disagree by a few px at the seam) with components <200 px dropped. Verdicts stamp as
`mask-ok-face-separated` / `mask-ok-front-figure-removed`. **No extra SAM calls** — still exactly
one per figure.

**Both halves are required.** The negative point alone leaves the front figure's legs and sandal
in the mask; the subtraction alone eats the target's head. Faces are required too — geometry-only
anchors put the positive point on background.

**There were TWO box→mask loops, and the failing page ran through the other one.** Its
`detectionBackend` is `gemini-second-opinion` (DINO found 4 persons against 5 expected, Gemini
found 5, arbitration chose Gemini), so masks came from `attachSamMasksToFigures`, not DINO Stage 2.
A fix in Stage 2 alone would never have fired on the page it was written for. A test asserts
`_mobilesamMaskFull` keeps exactly one call site so a third loop cannot reappear silently.

Also corrected: SAM is **deterministic** here — the identical call run 3× differed by 0 px. An
earlier "instability" reading was the head-leak interacting with dilation, not randomness.

**Verified end-to-end** by calling the real shipped `attachSamMasksToFigures` against a local
analyzer with that page's five stored figures: Daniel and Emma both come out clean and correctly
separated; Noah, Sarah and Hans are unchanged.

**Touched:** `server/lib/figureDetection.js` (`SAM_FRONT_*`, `SAM_SPECKLE_MIN_PX`,
`SAM_FACE_CONTAINMENT`, `_boxAreaPx`, `_boxContainment`, `_unionDilated`, `_dropSmallComponents`,
`_assignFacesToBoxes`, `_maskBoxesFrontFirst` + both call sites, `points`/`point_labels` on
`_mobilesamMaskFull`, `minCoverage` option on `_cleanMaskAndCheck`),
...
## 2026-08-15 — Trial skips text refinement (90s of every trial was a discarded wait)

**Context:** measured on both staging trial showcases
(`job_1786815617426`, `job_1786818831439`). Timeline of the second:
outline 0–98s, avatars 98–104s, **all 5 page images generated by 119s (7.2s of
pure generation)** — then the job sat until 209s waiting on the background text
refinement, which hit the 90s join cap, logged
`text_refine_join_timeout`, and shipped the ORIGINAL text. 90s of a 232s run
(39%), plus two refinement rounds of tokens, for output that was thrown away.
Both runs behaved identically (timeout at 192s and 209s).

**Root cause:** the refinement is started in parallel with images on the premise
(comment at the start site) that "images take ~25 min on a 10-page story,
refinement ~2-4 min" — true for full stories, where the polish pass hides behind
a long image phase. A trial's image phase is ~15 seconds, so refinement can
never finish behind it. Nothing excluded trial: `refineEnabled` was a plain
global env check.

**Decision:** `refineEnabled` also requires `!inputData.trialMode`. Full stories
are unchanged. Expected trial wall clock after this: ~140s (production baseline
is 123s).
