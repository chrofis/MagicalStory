...
| story-text-proofread.txt | storyHelpers.js `buildTextProofreadPrompt` | THE LECTOR, last step of the text chain: objective language faults in the final text, and it returns the corrected pages itself in `---STORY TEXT---` / `## Page n` form. Each returned page is screened by a per-page edit-distance cap (`screenLectorPages`, 15%). The separate apply pass (story-text-lector-apply.txt) was deleted 2026-09-03 |
