...
- **Style repair runs on GROK** (`styleRepairModel`), settled 2026-08-24 after grok→gemini→grok. Gemini REFUSES a photoreal adult face outright (`IMAGE_OTHER` ×3, two pages, plus Lab #837), so the page keeps its photograph; Grok repaints it correctly in ~18s/$0.02. The old "Grok no-ops" evidence was measured on pages already near the style and does not cover photographic ones — and a no-op is now rejected by the comparative gate anyway. Third flip: bring new evidence on PHOTOGRAPHIC pages, not near-style ones.
