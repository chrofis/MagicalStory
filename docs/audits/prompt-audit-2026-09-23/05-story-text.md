...
- FIXED (owner round 2, decisions.md 2026-09-23 "Text stage, owner round 2"): #1 the pass after the repair is a grammar check that edits numbered sentences only (FIX within 4 words / RESTORE verbatim) and cannot write a sentence neither version had; #2 the audit asks whether each arc hint was applied (HINT p6 found on the replay); B clothing kept but shrunk — writer, audit and refine read a compact spec (outfit version per page in short form), spec 16.8k → 11.7k; #6 1st-grade band 4-9, a sentence finding only past 13.
- DONE 2026-09-24: the Lab recall check of the blind slot (Lab #1454/#1455): recall 10/23 on 4 stored writer texts vs 16/23 for the stored arc-informed audit; it alone caught 4 real faults on this run's writer text; kept 🟡 — see decisions.md 2026-09-24 "Blind text audit recall measured".
