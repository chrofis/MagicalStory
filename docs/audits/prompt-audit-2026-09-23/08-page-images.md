...
- FIXED in the page-images round (2026-09-23, second evening):
  - S1 bbox coordinates in an inpaint instruction — `a3f7afb3a` (consolidator rule 3; replayed on the stored p12 prompt, no coordinates).
  - S4 "shallow depth of field" in the rewrite — `f604625ef` (`NO_LENS_RULE` in all four brief authors, anchored in art-director-vs-iterate).
  - S5 char-fix loses the held prop — `ec3a815d5` (state from `interactions[]`, `looksAt`, `perspective`). The SEAM had a second cause: the pipeline passed the face box as the figure box, so the large-face-box guard turned every pipeline face repair into a body crosshatch clipped to face box + 10% (p14 blend mask 439×517 = that box); same commit passes the figure box.
  - S7 crop artefacts — `391160c14` (`cutout_artifact` takes no repair route: not inpaintable, skipped by both char-fix gates; it already cost 0). The crop itself is unchanged (BACKLOG).
  - S9 / S10 plates — `0e2453313`: the SHOT line owns the camera; "direction named above" lines only for facts that name one; retries judged with vision; derived plates edited in the art style and QC-judged at their own shot with one re-derive; QC medium + camera checks; figure rule aligned with the generator (unnamed figures FAIL, a named crowd is fine). Replay: p12 derive FAIL (medium, figures), p14 base PASS.
  - S11 style gate echo — `63aecbdf8` (a copied observation is stored as `echoed`, `matches_style: null`; 6/6 flagged on replay).
  - S12 gaze at the page's own location — `b589e3443` (`vbIdGuard.gazeTarget`, shared by the page prompt, the judges and the char-fix).
  - S13 consolidator audit-only score — `a3f7afb3a` (2.1k deleted; the other 7k of that section is the `deduped_issues` contract the score reads).
  - Improve 9 unstored prompts — `be82b63cc` (`eval_calls`, migration 041: quality, semantic, inventory, plate QC, iterate re-brief).
- OPEN: S3 outfit-restatement dedupe (AD/wardrobe); S4 the rewrite's free-text self-report and its LOC-variant switch (`carryParentObjects` compares base ids, so `LOC002.5` → `LOC002.1` is kept); S7 a crop mask that keeps held objects; S12 the multi-actor pose row the prose contradicts (Art Director authoring — a row names only actors who do that action); S14.
- OWNER CALL: S2 version selection (second sample); S6 upstream eye colour; S11 a truly blind gate (move the three observations into the blind inventory call, compare in the quality judge); S12 HEIGHT ORDER vs age block = audit 04 A9 (the "category first" option was built, replayed and withdrawn: `586c7d95f`); S1 cap ranking by verified × severity.
