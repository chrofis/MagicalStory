...
- **Landmark photo variants reach Claude**: `storyIdeas.js` maps `photo_url_N + photo_description_N` from `landmark_index` into `photoVariants` on each landmark. The trial landmarks instruction (`buildTrialStoryPrompt`) lists what each photo shows (`PHOTOS` block, ≥2 variants), and the trial scene hint's `landmarkView` picks the photo by kind (`pickVariantForView`) — never a `[LOC###.N]` number, which is a vantage id (docs/decisions.md 2026-09-24). Do NOT add specific landmark names as examples in the prompt — Claude pattern-matches the example and picks it deterministically on every story in that city.
