...
describe('buildReferenceSheetPrompt / batches with the new cells', () => {
  ...
  it('a multi-cell sheet carries the kind sentences and the blanket no-text rule', () => {
    ...
    // Two cells lay out as a 2x2 with a mirrored map since 2026-09-14
    // (referenceSheetLayout) — the bottom row repeats the top row.
    expect(p).toContain('Top-left: This is the small dragon scale, a single reptile scale: ');
    expect(p).toContain('Top-right: This is the large dragon scale, a single reptile scale: ');
