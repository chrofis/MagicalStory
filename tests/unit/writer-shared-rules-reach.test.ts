...
//
//   PAGE_OPENING_VARIETY_RULE → beats text writer, trial, both unified variants
//   AD_COMPOSITION_RULE       → trial and both unified variants (the writers
//                               that author scene hints with no Art Director);
//                               NOT the beats text writer, which stages nothing.
...
beforeAll(async () => {
  ...
  built = {
    ...
    imageFirst: buildUnifiedStoryPrompt(input({ storyPromptVariant: 'imageFirst' }), 12),
    textFirst: buildUnifiedStoryPrompt(input({ storyPromptVariant: 'textFirst' }), 12),
  };
});

describe('page-opening variety rule reaches every stage that writes page prose', () => {
  it.each(['beats', 'trial', 'imageFirst', 'textFirst'])('%s', (name) => {
  ...
  it('is a bare sentence, so each template supplies its own list marker', () => {
    ...
    for (const name of ['trial', 'imageFirst', 'textFirst']) {
...
describe('Art Director composition rules reach every writer that authors scene hints', () => {
  it.each(['trial', 'imageFirst', 'textFirst'])('%s', (name) => {
  ...
  it('replaced, not duplicated, the unified twins\' own focal-point bullets', () => {
    for (const name of ['imageFirst', 'textFirst']) {
      const hits = built[name].match(/One moment, one focal point/g) || [];
