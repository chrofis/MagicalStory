...
describe('wornItems — METADATA parser', () => {
  it('parses a wornItems block out of a prose+METADATA brief', () => {
    ...
    expect(m.wornItems).toEqual([
      { id: 'ART001', owner: 'Lily', state: 'off', location: 'lies on the cobbles beside the log pile', wearer: null, redressNote: null },
    ]);
  ...
  it('drops rows with no usable id and blanks an unrecognised state', () => {
    ...
    expect(rows).toEqual([
      { id: 'ART001', owner: 'Lily', state: 'worn', location: null, wearer: null, redressNote: null },
      { id: 'ART002', owner: 'Lily', state: null, location: null, wearer: null, redressNote: null },
    ]);
  });
