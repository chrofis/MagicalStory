...
describe('trimVbAssignments', () => {
  it('brings every over-budget page down to the budget, plan-line-named entries first', () => {
    ...
    const report = trimVbAssignments(vb, PLAN, { castNames: ['the girl'] });
    expect(report.pagesOverBudgetBefore).toBe(1);
    expect(report.pagesOverBudgetAfter).toBe(0);
    expect(count(vb, 3)).toBe(VB_ELEMENT_BUDGET);
  ...
  it('never counts or strips a location, real or invented, even on a full range (owner, 2026-09-08)', () => {
    ...
    expect(trimVbAssignments(solo, PLAN).pagesOverBudgetBefore).toBe(0);
  });

  it('hands the page its location cell LAST, on top of the budget, never above the Grok slot cap', () => {
    ...
    expect(refs.map((r: any) => r.id)).toEqual(['CHR001', 'ART001', 'ART002', 'LOC002']);
    expect(refs).toHaveLength(VB_ELEMENT_BUDGET + 1);
    expect(VB_ELEMENT_BUDGET + 1).toBeLessThanOrEqual(VB_SLOT_MAX_ELEMENTS);
    expect(refs[refs.length - 1].type).toBe('location');
  });

  it('never strips an entry the plan line names in favour of an unnamed one', () => {
    const vb = bible();
    trimVbAssignments(vb, PLAN, { castNames: ['the girl'] });
    const kept = ids(vb, 3);
  ...
  it('never touches a real landmark', () => {
    ...
    expect(count(vb, 1)).toBe(VB_ELEMENT_BUDGET);
  });
  ...
  it('is a no-op on an empty or missing bible and never throws', () => {
    expect(trimVbAssignments(null, PLAN)).toEqual({ pagesOverBudgetBefore: 0, pagesOverBudgetAfter: 0, stripped: [], droppedStates: [] });
